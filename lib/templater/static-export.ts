...
*{box-sizing:border-box}body{margin:0;background:var(--canvas);color:var(--text);font-family:${cssFont(theme.typography.body)}}a{color:inherit;text-decoration:none}.wrap{max-width:var(--max-width);margin:0 auto;padding:0 28px}.section{padding:56px 0}.compact{padding:28px 0}.spacious{padding:86px 0}.surface{background:var(--surface)}.dark{background:var(--text);color:#fff}.primary{background:var(--primary);color:#fff}.announcement{padding:10px 18px;background:var(--text);color:#fff;text-align:center;font-size:13px;font-weight:700}.header{position:sticky;top:0;z-index:5;border-bottom:1px solid var(--border);background:color-mix(in srgb,var(--surface) 94%,transparent);backdrop-filter:blur(12px)}.header-inner{display:flex;align-items:center;justify-content:space-between;gap:28px;min-height:68px}.logo{font-weight:900;letter-spacing:.16em}.nav{display:flex;gap:24px;color:var(--muted);font-size:14px;font-weight:650}.pill{display:inline-flex;border:1px solid var(--border);border-radius:999px;padding:9px 15px;background:var(--surface);font-size:13px;font-weight:750}h1,h2,h3{margin:0;font-family:${cssFont(theme.typography.heading)};line-height:1.05;letter-spacing:0}h1{font-size:calc(56px * var(--heading-scale))}h2{font-size:calc(34px * var(--heading-scale))}p{margin:0}.eyebrow{color:var(--primary);text-transform:uppercase;letter-spacing:.18em;font-size:12px;font-weight:900}.muted{color:var(--muted)}.button{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:var(--radius);background:var(--primary);color:#fff;padding:13px 20px;font-weight:900;text-decoration:none;box-shadow:0 14px 28px rgb(15 23 42 / .12)}.button.dark{background:var(--text)}.button.outline{border:1px solid var(--border);background:var(--surface);color:var(--text)}.hero-grid{display:grid;grid-template-columns:.92fr 1.08fr;gap:48px;align-items:center}.hero-grid.spotlight{grid-template-columns:1.12fr .88fr}.hero-centered{text-align:center}.hero-centered .hero-copy{margin-left:auto;margin-right:auto}.hero-copy{margin-top:22px;max-width:620px;color:var(--muted);font-size:18px;line-height:1.7}.actions{display:flex;gap:12px;flex-wrap:wrap;margin-top:30px}.hero-centered .actions{justify-content:center}.hero-card{position:relative;min-height:480px;overflow:hidden;border:1px solid var(--border);border-radius:calc(var(--radius) + 12px);background:var(--surface);box-shadow:0 28px 60px rgb(15 23 42 / .14)}.hero-art{position:absolute;inset:18px;border-radius:var(--radius);background:linear-gradient(135deg,var(--secondary),var(--accent))}.floating-card{position:absolute;left:34px;right:34px;bottom:34px;border-radius:var(--radius);background:rgb(255 255 255 / .86);color:var(--text);padding:22px;box-shadow:0 20px 45px rgb(15 23 42 / .16)}.hero-products{margin-top:34px}.grid{display:grid;gap:22px}.cols-2{grid-template-columns:repeat(2,minmax(0,1fr))}.cols-3{grid-template-columns:repeat(3,minmax(0,1fr))}.cols-4{grid-template-columns:repeat(4,minmax(0,1fr))}.product-grid{display:grid;gap:22px;margin-top:32px}.product-grid.grid{grid-template-columns:repeat(3,minmax(0,1fr))}.product-grid.editorial{grid-template-columns:repeat(3,minmax(0,1fr))}.product-grid.editorial>*:first-child{grid-column:span 2}.product-grid.compact{grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.card{border:1px solid var(--border);border-radius:calc(var(--radius) + 8px);background:var(--surface);overflow:hidden;box-shadow:0 14px 28px rgb(15 23 42 / .06)}.card.compact .card-body{padding:12px}.card.compact h3{font-size:15px}.card-body{padding:18px}.product-image{aspect-ratio:4/5;background-color:#f8fafc;background-position:center;background-size:cover;background-repeat:no-repeat}.card.compact .product-image{aspect-ratio:1}.tile-art{height:150px;background:linear-gradient(135deg,var(--secondary),var(--accent))}.trust-item{display:flex;gap:12px;align-items:center;border-radius:var(--radius);background:var(--canvas);padding:16px;font-weight:800}.summary{border:1px solid var(--border);border-radius:calc(var(--radius) + 8px);background:var(--surface);padding:22px}.line{display:flex;justify-content:space-between;gap:18px;padding:10px 0;color:var(--muted)}.footer{padding:46px 0;background:var(--text);color:#fff}@media (max-width:760px){.wrap{padding:0 20px}.section{padding:42px 0}.nav{display:none}h1{font-size:calc(38px * var(--heading-scale))}h2{font-size:calc(28px * var(--heading-scale))}.hero-grid,.hero-grid.spotlight,.cols-2,.cols-3,.cols-4,.product-grid.grid,.product-grid.editorial{grid-template-columns:1fr}.product-grid.compact{grid-template-columns:repeat(2,minmax(0,1fr))}.product-grid.editorial>*:first-child{grid-column:span 1}.hero-card{min-height:320px}.floating-card{left:20px;right:20px;bottom:20px}}`;
}
...
function renderStaticSection(section: StaticSection, context: StaticContext) {
  ...
  if (section.type === "hero") {
    return renderStaticHero(section, context);
  }
...
function renderStaticHero(section: StaticSection, context: StaticContext) {
  const settings = section.settings;
  const variant = styleSetting(settings.variant, "split");
  const { pages, template } = context;
  const copy = `<div><p class="eyebrow">${escapeHtml(String(settings.eyebrow ?? ""))}</p><h1 style="margin-top:16px">${escapeHtml(String(settings.title ?? ""))}</h1><p class="hero-copy">${escapeHtml(String(settings.copy ?? ""))}</p><div class="actions"><a class="${buttonClasses(settings)}" href="${hrefForPageType(pages, "collection")}">${escapeHtml(String(settings.cta ?? "Shop now"))}</a><a class="button outline" href="${hrefForPageType(pages, "about")}">View lookbook</a></div></div>`;

  if (variant === "centered") {
    return `<section class="${sectionClasses(section, "spacious")}"><div class="wrap hero-centered">${copy}<div class="product-grid grid hero-products">${template.products.slice(0, 3).map((product) => renderProductCard(product, hrefForPageType(pages, "product"))).join("")}</div></div></section>`;
  }

  if (variant === "productSpotlight") {
    return `<section class="${sectionClasses(section, "spacious")}"><div class="wrap hero-grid spotlight">${renderSpotlightCard(context.activeProduct ?? template.products[0])}${copy}</div></section>`;
  }

  return `<section class="${sectionClasses(section, "spacious")}"><div class="wrap hero-grid">${copy}${renderHeroArtCard()}</div></section>`;
}

function renderHeroArtCard() {
  return `<div class="hero-card"><div class="hero-art"></div><div class="floating-card"><p class="eyebrow">Featured set</p><h3 style="margin-top:10px">Curated everyday essentials</h3><p class="muted" style="margin-top:8px;line-height:1.5">Three-piece capsule bundle from $164.</p></div></div>`;
}

function renderSpotlightCard(product?: Product) {
  return `<div class="hero-card" style="background-image:${cssValue(product?.image ?? "")};background-position:${product?.imagePositionX ?? 50}% ${product?.imagePositionY ?? 50}%;background-size:${product?.imageZoom ?? 100}%;background-repeat:no-repeat"><div class="floating-card"><p class="eyebrow">Spotlight</p><h3 style="margin-top:10px">${escapeHtml(product?.name ?? "Featured product")}</h3><p class="muted" style="margin-top:8px;line-height:1.5">${product ? `$${product.price} / ${escapeHtml(product.category)}` : "Ready to customize."}</p></div></div>`;
}

function renderProductCard(product: Product, href: string, compact = false) {
  return `<article class="card${compact ? " compact" : ""}"><a href="${href}"><div class="product-image" style="background-image:${cssValue(product.image)};background-position:${product.imagePositionX ?? 50}% ${product.imagePositionY ?? 50}%;background-size:${product.imageZoom ?? 100}%"></div><div class="card-body"><p class="muted" style="text-transform:uppercase;letter-spacing:.12em;font-size:12px;font-weight:800">${escapeHtml(product.category)}</p><h3 style="margin-top:10px">${escapeHtml(product.name)}</h3><p style="margin-top:14px;font-weight:900">$${product.price}</p></div></a></article>`;
}
